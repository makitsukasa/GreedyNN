	...
	def train(self, n_epoch, batch_size=64):
		...
		for epoch in range(n_epoch):
			for iteration in range(n_batches):
				...
				# 近似
				theta, _, _, _ = np.linalg.lstsq(x, gen_imgs_fitness.flatten(), rcond = None)
				fitness_pred_error = np.copy(gen_imgs_fitness)
				for i in range(x_len):
					fitness_pred_error -= np.reshape(theta[i] * x[:,i], fitness_pred_error.shape)
